...
int main()
{
    ...
    glRenderbufferStorage(
        GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, SCR_WIDTH,
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        // now bind back to default framebuffer and draw a quad plane with the attached framebuffer color
        // texture
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
        glDisable(
            GL_DEPTH_TEST);   // disable depth test so screen-space quad isn't discarded due to depth test.
        // clear all relevant buffers
        glClearColor(1.0f, 1.0f, 1.0f, 1.0f);   // set clear color to white (not really necessary actually,

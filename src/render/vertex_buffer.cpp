                                  ...
                                  uint32_t const vcount)
{
    ...
    if (m_components[ComponentsBitPos::normal]) {
        ...
        if (index < m_vertex_count) {
            new_dynamic_buffer.insert(new_dynamic_buffer.end(),
                                      m_dynamic_buffer.begin() + old_pos_block_total_floats + index * norm_floats_per_vertex,
                                      m_dynamic_buffer.begin() + old_pos_block_total_floats + m_vertex_count * norm_floats_per_vertex);
        }
    }
    ...
    if (m_components[ComponentsBitPos::tex]) {
        ...
        uint32_t static_capacity_estimation = new_total_vertices * tex_floats_per_vertex * m_tex_channels_count;
        new_static_buffer.reserve(static_capacity_estimation);

        for (uint32_t i = 0; i < m_tex_channels_count; ++i) {
            assert(tex[i]);
            // Calculate the start of the data block for the current texture channel 'i' in the old m_static_bufffer.
            // Each channel's data is contiguous: T0_data T1_data ...
            // So, channel 'i' starts after (i * m_vertex_count * tex_floats_per_vertex) floats.
            ...
            new_static_buffer.insert(new_static_buffer.end(),
                                     ...
                                     m_static_bufffer.begin() + old_tex_channel_block_start_offset + index * tex_floats_per_vertex);
            // Insert new tex coords for this channel
            new_static_buffer.insert(new_static_buffer.end(), tex[i], tex[i] + vcount * tex_floats_per_vertex);
            // Copy existing tex coords for this channel, after the insertion point 'index' (if any)
            if (index < m_vertex_count) {
                new_static_buffer.insert(new_static_buffer.end(),
                                         m_static_bufffer.begin() + old_tex_channel_block_start_offset + index * tex_floats_per_vertex,
                                         m_static_bufffer.begin() + old_tex_channel_block_start_offset + m_vertex_count * tex_floats_per_vertex);
            }
        }
...
{
    assert(pos);
    if (icount > 0) { // Ensure indices is valid if we are going to use it.
        assert(indices);
    }
    if (vcount == 0) return; // Nothing to add if no vertices
    ...
    if(m_components[ComponentsBitPos::tex])
    {
		...
		for (uint32_t i = 0; i < m_tex_channels_count; ++i) {
            ...
            uint32_t offset_floats = 
                (i * m_vertex_count * tex_floats_per_vertex) + 
...
void VertexBuffer::eraseVertices(uint32_t const first, uint32_t const last)
{
	...
	if (first >= m_vertex_count || first >= last) { // If 'first' is out of bounds or range is invalid/empty.
        return;
    }
    ...
    if (m_components[ComponentsBitPos::normal]) {
        ...
        // Original start of N_block was at m_vertex_count * pos_floats_per_vertex (relative to old dynamic_buffer start).
        // After erasing 'count_to_erase' positions, the P_block is now (m_vertex_count - count_to_erase) * pos_floats_per_vertex long.
        // This is the new offset from dynamic_buffer.begin() to where the N_block data starts.
        uint32_t n_block_new_start_offset = (m_vertex_count - count_to_erase) * pos_floats_per_vertex;
        
        auto n_erase_begin_it = m_dynamic_buffer.begin() + n_block_new_start_offset + first * norm_floats_per_vertex;
        auto n_erase_end_it = m_dynamic_buffer.begin() + n_block_new_start_offset + last * norm_floats_per_vertex;
    ...
    if (m_components[ComponentsBitPos::tex]) {
        ...
        for (uint32_t i = 0; i < m_tex_channels_count; ++i) {
            ...
            auto t_erase_begin_it = m_static_bufffer.begin() + channel_i_original_start_offset +
                                   first * tex_floats_per_vertex - total_tex_floats_erased_from_previous_channels;
...
void VertexBuffer::updateDynamicBuffer(std::vector<float> pos, std::vector<float> norm)
{
	...
	new_dynamic_buffer.push_back(std::move(norm));
